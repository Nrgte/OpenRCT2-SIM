...
#pragma warning(default : 6011)
    ...
    void Guest::initAGS(std::vector<RideId> rides)
    {
        ...
        if (!(this->RideSubState == PeepRideSubState::onRide))
            ...
            if (currentRide->getRideTypeDescriptor().flags.has(RtdFlag::isTransportRide))
                PathFinding::InitializePathFinding(*this);
    }
    ...
    bool Guest::IsStillOnRide()
    {
        ...
        Vehicle* currentTrain = getGameState().entities.GetEntity<Vehicle>(currentRide->vehicles[CurrentTrain]);
        if (!currentTrain)
            return false;
