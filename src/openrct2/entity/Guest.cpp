...
namespace OpenRCT2
{
    ...
    void Guest::tick128UpdateGuest(uint32_t index)
    {
        ...
        // The happier the guest the more likely they go on a ride. Base Happiness of 160 = 8192. Or if the guest is sick or has
        // to go to the toilet. Each failed try increases the success chance of subsequent tries;
        float failedGoOnRideChecksMultiplier = 1.3f;
        float failedModifier = pow(failedGoOnRideChecksMultiplier, FailedGoOnRideChecks + 1) + 1
            - failedGoOnRideChecksMultiplier;
    ...
    void Guest::onExitRide(Ride& ride)
    {
        ...
        // std::string rideValue = "Guest Ride Rating for Ride: " + ride.GetName() + ": " +
        // ride.GetRideTypeDescriptor().EnumName
    ...
    static Ride* GuestFindBestRideToGoOn(Guest& guest)
    {
            ...
            if (rideConsideration.size() > rideIndex && rideConsideration[rideIndex])
            {
                if (!(ride.flags.has(RideFlag::queueFull)))
                {
                    if (guest.shouldGoOnRide(ride, StationIndex::FromUnderlying(0), false, true)
                        && (RideHasRatings(ride) || ride.getRideTypeDescriptor().flags.has(RtdFlag::isShopOrFacility)))
                    {
                        const auto& rtd = ride.getRideTypeDescriptor();
                        if (guest.toilet >= toiletThreshold && !(rtd.specialType == RtdSpecialType::toilet))
                            continue;
                        else if (guest.nausea >= nauseaThreshold && !(rtd.specialType == RtdSpecialType::firstAid))
                            continue;
                        else if (guest.hunger <= hungerThreshold)
                        {
                            if (!rtd.flags.has(RtdFlag::sellsFood))
                                continue;
                        }
                        else if (guest.thirst <= thirstThreshold)
                        {
                            if (!rtd.flags.has(RtdFlag::sellsDrinks))
                                continue;
                        }
                        else if (guest.cashInPocket <= cashInPocketThreshold)
                        {
                            if (!(rtd.specialType == RtdSpecialType::cashMachine) || guest.happiness < 200)
                                continue;
                        }
                        else if (
                            guest.toilet < toiletThreshold && guest.hunger > hungerThreshold && guest.thirst > thirstThreshold
        ...
        if (guest.toilet >= toiletThreshold || guest.thirst <= thirstThreshold || guest.hunger <= hungerThreshold
            || guest.cashInPocket <= cashInPocketHardThreshold)
        {
        ...
        else if (guest.nausea > nauseaThreshold)
            lookingForFacitilty = true;
        ...
        std::vector<std::pair<Ride*, RideRating_t>> rideExcitments;
        ...
        if (viableRides.size() > 0)
            for (Ride* ride : viableRides)
            {
                ...
                float medianIntensityRating = guest.AGS->GetMedianIntensityRating(ride->id, ride->ratings.intensity);
                // if (!HasRidden(*ride))
                ...
                std::span<RideStation> stations = ride->getStations();
                ...
                for (RideStation& station : stations)
                {
                    ...
                    int32_t pfScore = PathFinding::CalculateHeuristicPathingScoreWrapper(guest.getLocation(), stationPosition);
        ...
        std::sort(
            rideExcitments.begin(), rideExcitments.end(), [](const auto& a, const auto& b) { return a.second > b.second; });
        std::sort(
            rideQueueTimes.begin(), rideQueueTimes.end(), [](const auto& a, const auto& b) { return a.second < b.second; });
    ...
    bool Guest::shouldGoOnRide(Ride& ride, StationIndex entranceNum, bool atQueue, bool thinking)
    {
        ...
        if (ride.status == RideStatus::open && !ride.flags.has(RideFlag::brokenDown))
        {
                ...
                || RideGetPrice(ride) != 0)
            {
                ...
                if (peepFlags.has(PeepFlag::leavingPark) && this->getNextProxyRide() != &ride)
                {
                ...
                || ridePrice != 0)
            {
                ...
                if (ridePrice != 0 && !GuestHasVoucherForFreeRide(*this, ride) && !gameState.park.flags.has(ParkFlag::noMoney))
                {
                    if (ridePrice > cashInPocket)
                    {
                        if (peepAtRide)
                        {
                            ...
                            if (this->peepFlags.has(PeepFlag::leavingPark) && this->getNextProxyRide() == &ride)
                            {
                ...
                if (RideHasRatings(ride))
                {
                    ...
                    {
                        const bool isPrecipitating = Weather::isPrecipitating();
                        if (isPrecipitating && !GuestShouldRideWhileRaining(*this, ride) && this->getNextProxyRide() != &ride)
                        {
                            if (peepAtRide)
                            {
                        ...
                        // Also skip intensity checks if the ride is used as a proxy ride and is a transport ride to get to
                        // another ride.
                                ...
                                && !ride.getRideTypeDescriptor().flags.has(RtdFlag::isTransportRide)))
                        {
                            if (!gameState.cheats.ignoreRideIntensity)
                            {
                                ...
                                RideRating_t maxIntensity = (intensity.getMaximum() * 100);
                                RideRating_t minIntensity = (intensity.getMinimum() * 100);
                                ...
                                if (ride.ratings.intensity < minIntensity - happiness
                                    && !ride.getRideTypeDescriptor().flags.has(RtdFlag::isTransportRide) && nausea < 160)
                                {
                                    if (peepAtRide)
                                    {
                                ...
                                float averageIntensityRating = this->AGS->GetMedianIntensityRating(
                                    ride.id, ride.ratings.intensity);
                    ...
                    && (ScenarioRand() & 0xFFFF) < this->AGS->GetQueueTimeCancelChance())
                {
                    if (peepAtRide)
                    {
                        insertNewThought(PeepThoughtType::queuingAges, ride.id);
                        if (happinessTarget >= 64)
                        {
                            happinessTarget -= 8;
                        }
                ...
                int32_t averageRideSpeed = ((ride.averageSpeed * 9) >> 18) * 1.60934;
                ...
                if (rideTime > 0)
                {
                    ...
                    // function graph2:
                    // y\ =\sqrt{\left(x^{2}\ \cdot\left(\frac{\sqrt{L\cdot2}}{\sqrt{24}}\right)\cdot\left(\frac{z}{90}\right)\right)}
                    // f log dist: f\left(L,z,\ x\right)\ =\ \left\{x\ <30:\ 3\cdot
                    // r^{2}-2r^{3},\ 1\right\}\ \cdot\log_{t}\left(mx^{n}\right)\ +\ \frac{\left(\frac{L^{o}}{24}\ \cdot\ \frac{z^{q}}{90}\right)}{b+\left(\frac{1}{z+L}\right)}
                    // f exp s bend:
                    // f\left(L,z,\ x\right)\ =\ u\left(\frac{1}{(1+\exp(-kx))}\right)^{a}\ \cdot\ \frac{\left(\frac{L^{o}}{24}\ \cdot\
    ...
    static bool GuestShouldGoOnRideAgain(Guest& guest, const Ride& ride)
    {
        ...
        if (guest.getNextProxyRide() == &ride)
            return false;
        ...
        */
        ...
        if (guest.AGS->GetMedianIntensityRating(ride.id, ride.ratings.intensity) >= 7)
            return true;
    ...
    static void GuestDecideWhetherToLeavePark(Guest& guest)
    {
        ...
        if (!guest.peepFlags.has(PeepFlag::leavingPark))
        {
            ...
            else
            {
                ...
                else if (guest.energy <= 55 && guest.happiness <= 45 && guest.cashInPocket <= 1.00_GBP)
    ...
    bool Guest::PeepGoToNewCar(const Ride& ride, int16_t xLocal, int16_t yLocal, int16_t zLocal)
    {
        ...
        return this->PeepGoToNewCar(ride);
    }

    bool Guest::PeepGoToNewCar(const Ride& ride)
    {
        if (ride.status != RideStatus::open || ride.vehicleChangeTimeout != 0)
            return false;
    ...
    void Guest::UpdateRideWaitForTrain()
    {
        ...
        if (this->currentTrain == OpenRCT2::Limits::kMaxTrainsPerRide && trainAtStation != this->currentTrain)
        {
            this->currentTrain = trainAtStation;
        ...
        if (!carEntry->flags.has(CarEntryFlag::loadingWaypoints))
        {
            ...
            if (!ride->getRideTypeDescriptor().flags.has(RtdFlag::vehicleIsIntegral))
            {
                ...
                if (!shouldExitStation)
                    shouldExitStation = !this->PeepGoToNewCar(
                        *ride, platformLocation.x, platformLocation.y, platformLocation.z);
...
#pragma warning(default : 6011)
    ...
    Guest* Guest::generate(const CoordsXYZ& coords)
    {
        ...
        if (gameState.park.flags.has(ParkFlag::guestPreferLessIntenseRides))
        {
            ...
            {
                intensityRange = 4;
                intensityOffset = ScenarioRand() & 0x2 % 2 ? offsetParkPrefLessIntenseRides : offsetParkPrefMoreIntenseRides;
            }
            else
            {
                intensityOffset = offsetParkPrefLessIntenseRides;
            }
        }
    ...
    void Guest::RateRide(Ride& ride)
    {
        ...
        float randomIntensityProbability = distribution(generator);
        totalProbability = 0;
        for (uint8_t i = minIntensity; i <= maxIntensity; i++)
        {
            totalProbability += probRatingMatrix[i];
            if (totalProbability > randomIntensityProbability)
            {
                //std::normal_distribution<float> normalDist(probRatingMatrix[i] * 10, standardDistDeviation);
                //rating = normalDist(generator);
                rating = (normal_pdf(ride.intensity / 100.0f, i, standardDistDeviation) / 0.2f) * 10;
                rating++; // Rounding up since a 10 is almost impossible.
                break;
            }
        }

        */
        // Main function if the intensity is within the parameters the guest likes.
        if (ride.ratings.intensity / 100.0f > minIntensity && ride.ratings.intensity / 100.0f < maxIntensity)
        {
            rating = (tampered_function(minIntensity - 0.1f, maxIntensity + 0.1f, (ride.ratings.intensity / 100.0f)) / 1) * 10;
        }
        else
        {
            rating = getRideRatingProbability(
                         ride.ratings.intensity / 100.0f, standardDeviationM, (averageIntensity - standardDeviationM) * -1)
                * 10;
        }
        rating = ((rating * 2) + (ride.ratings.excitement / 100.0f)) / 3;
        rating++;
        ...
        if (rideRatings.size() > 1)
        {
            ...
            {
                std::string rat = std::to_string(rideRatings[i].getRating());
                std::string rideName = rideRatings[i].getRide()->GetName();
                std::cout << this->GetName() << " " << i << "th rating for " << rideName << " : " << rat << std::endl;
            }
    ...
    void Guest::initAGS(std::vector<RideId> rides)
    {
        ...
        Ride* cr = GetRide(this->currentRide);
        if (!(this->rideSubState == PeepRideSubState::onRide))
            PathFinding::InitializePathFinding(*this);
        else if (cr)
            if (cr->getRideTypeDescriptor().flags.has(RtdFlag::isTransportRide))
                PathFinding::InitializePathFinding(*this);
    }
    ...
    {
        this->guestHeadingToRideId = ride.id;
        this->guestIsLostCountdown = 200;
        this->resetPathfindGoal();
    ...
    bool Guest::IsStillOnRide()
    {
        ...
        Vehicle* ct = getGameState().entities.getEntity<Vehicle>(currentRide->vehicles[currentTrain]);
        if (!currentTrain)
            return false;
        ...
        Vehicle* currentCar = ct->GetCar(this->currentCar);
        if (!currentCar)
            return false;
    ...
    float getRideRatingProbability(uint8_t value, float standardDeviation, int8_t offset)
    {
        ...
        //     (1 / standardDeviation * sqrt(2 * std::numbers::pi_v<float>)) * e, pow(-0.5f * ((value - mean) /
        //     standardDeviation),2));
    ...
    // RideIntensity = x
    // y=\frac{\left(x-i\right)^{\left(a-1\right)}\cdot\left(m-x\right)^{\left(a-1\right)}}{\left(m-i\right)}\cdot T
    // second function:
    // y=\frac{\left(x-i\right)^{\left(a-1\right)}\cdot\left(m-x\right)^{\left(a-1\right)}}{\left(m-i\right)^{t}}\cdot T
    double tampered_function(double min, double max, double rideIntensity)
    {
        /* Good values:
        const double a = 1.68f;
        const double T = 2.7f;
        const double n = 1.3f;
        */
        const double a = 1.81f;
        const double T = 2.0f;
        const double n = 1.81f;
    ...
    // f(x)=(1/(d\ *\ \sqrt{(2*\pi)}))*\exp(-((x-M)^{2})/8)
    // d = standard deviation, M = mean, peak is always ~0.2 with a standard deviation of 2
    double normal_pdf(double x, double mean, double std_dev)
    {
        return (1.0 / (std_dev * std::sqrt(2 * std::numbers::pi))) * std::exp(-0.5 * std::pow((x - mean) / std_dev, 2));
    }
    ...
    uint8_t getRandomTemperdFunctionValue(uint8_t highestMinIntensity)
    {
        std::map<uint8_t, float> probRatingMatrix;
        float totalProbability = 0;
        for (uint8_t i = 0; i <= highestMinIntensity; i++)
        {
            float probability = tampered_function(-0.1f, highestMinIntensity, i);
            totalProbability += probability;
            probRatingMatrix[i] = probability;
        }

        float randomProbability = getRandomUniformDistributionValue(0.0f, totalProbability);
        totalProbability = 0;
        for (uint8_t i = 0; i <= highestMinIntensity; i++)
        {
            totalProbability += probRatingMatrix[i];
