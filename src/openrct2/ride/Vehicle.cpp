...
void Vehicle::Update()
{
    ...
    switch (status)
    {
        ...
        case Vehicle::Status::unloadingPassengers:
            ...
            break;
        case Status::confirmUnloadingDone:
...
void Vehicle::UpdateMovingToEndOfStation()
{
    ...
    switch (curRide->mode)
    {
        ...
        case RideMode::circus:
            ...
            velocity = 0;
            acceleration = 0;
            SetState(Status::confirmUnloadingDone);
            break;
        default:
        {
...
void Vehicle::UpdateUnloadingPassengers()
{
    ...
    else
    {
             ...
             train = getGameState().entities.GetEntity<Vehicle>(train->next_vehicle_on_train))
        {
            ...
            {
                Guest* curPeep = getGameState().entities.GetEntity<Guest>(train->peep[peepIndex]);
                if (curPeep != nullptr)
                {
                    test = 0;
...
uint8_t Vehicle::GetGuestOnVehicleCount()
{
    ...
    for (uint8_t i = 0; i < num_seats_on_train; i++)
    {
        ...
        Guest* guest = getGameState().entities.GetEntity<Guest>(guestID);
        if (!guest)
            continue;

        Vehicle* headVehicle = getGameState().entities.GetEntity<Vehicle>(curRide->vehicles[guest->CurrentTrain]);
        if (!headVehicle)
        {
...
bool Vehicle::IsTrainReadyForBoarding() {
         ...
         trainCar = getGameState().entities.GetEntity<Vehicle>(trainCar->next_vehicle_on_train))
    {
        ...
        {
            Guest* guest = getGameState().entities.GetEntity<Guest>(trainCar->peep[i]);
