        ...
        for (auto peep : EntityList<Staff>())
        {
            ...
            if (distance < closestDistance)
            {
                ...
                std::deque<TileCoordsXYZ> tileList = AdvancedPathfinding::AStarSearch(
                    TileCoordsXYZ{ peep->getLocation() }, TileCoordsXYZ{ entrancePosition }, *peep, false);
                if (tileList.size() == 0)
                    continue;
...
#pragma endregion
    ...
    bool Ride::IsQueueFull(const RideStation& station)
    {
        ...
        {
            TileCoordsXYZ lastPeepInQueueTile = TileCoordsXYZ{ CoordsXYZ{ lastPeepInQueue->x, lastPeepInQueue->y,
                                                                          lastPeepInQueue->z } };
    ...
    money64 Ride::GetNormalizedRideValue() const
    {
        ...
        if (rideTime == 104)
            rideTime = 104;
        int32_t averageRideSpeed = ((this->averageSpeed * 9) >> 18) * 1.60934; // in km/h
        ...
        // money64 value2 = sqrt(
        //    (value * value) * sqrt(averageRideSpeed * 2) / sqrt(normalizedAverageRideSpeed) * rideTime /
        //    normalizedRideDuration);
        //    // Old function

        money64 newValue = amplitude * pow((1 / (1 + exp(-incline * oldValue))), falloff)
            * (pow(averageRideSpeed, averageRideSpeedWeight) / normalizedAverageRideSpeed * pow(rideTime, rideDurationWeight)
               / normalizedRideDuration)
